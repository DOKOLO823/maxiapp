...
import client from 'src/api/client'; // <-- Import direct Axios instance
...
const MiniAudioPlayer: FC<Props> = props => {
  const {onGoingAudio} = useSelector((state: any) => state.player);
      ...
      <View
        style={{
          ...
          width: `${mapRange({
            ...
            outputMax: 100,
            inputMin: 0,
            inputMax: onGoingAudio?.duration || 100, // Si track-player est supprimé
            inputValue: onGoingAudio?.position || 0,
          })}%`,
        }}
      />
